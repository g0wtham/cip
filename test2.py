...
j=1
i=1
while(os.stat('dummy'+str(i)+'.csv').st_size<>0):
	f=open('dummy'+str(i)+'.csv',"r")
	for r1 in csv.reader(f):
		text=r1[5]
	#	print text
		break
	f.close()
	f3=open('dummy'+str(i)+'.csv',"r")
	for r in csv.reader(f3):
		line=r[5]	
#take the first line of the file as your centroid
	#	open('','w').close()
	#	print line
		line=line.strip('\n')
			#for x in line:
			#	print x
		similar_index=similarity(text,line,True)
		if(similar_index>0.55):
			f1=open('outcsv'+str(i+1)+'.csv',"a")
			writer=csv.writer(f1)
			writer.writerow(r)
	#		print r[5]+str(i+1)
		else:
			f2=open('dummy'+str(i+1)+'.csv',"a")
